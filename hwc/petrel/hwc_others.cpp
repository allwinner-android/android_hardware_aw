...
static void release_external_display_acquire_fence() {
    SUNXI_hwcdev_context_t *Globctx = &gSunxiHwcDevice;
    unsigned long arg[4];
    ...
    arg[3] = 1;
    int fencefd = ioctl(Globctx->DisplayFd, DISP_HWC_COMMIT, (unsigned long)arg);
    close(fencefd);
    ALOGD("release external display acquire fence");
}
...
static int handle_unconnect_external_display(int disp, int out_type, int out_mode)
{
    ...
    release_external_display_acquire_fence();
