	...
	if(Globctx->hwcdebug & LAYER_DUMP)
	{
		...
		for(i = 0; i < Localctx->numberofLayer ; i++)
		{
            ...
            ALOGD("%5s|%2d|%3s| %f| %f| %02x| %08x| %08x| %08x| %08x| %02x| %05x| %08x|[%5d,%5d,%5d,%5d]|[%5d,%5d,%5d,%5d]|%s\n",
                    ...
                    handle == 0 ? 0 : ((handle->flags & private_handle_t::PRIV_FLAGS_USES_CONFIG) ? get_ion_address(handle->share_fd) : 0),
...
static unsigned int  CntOfLyaerMem(hwc_layer_1_t *psLayer)
{
    ...
    if(handle->format== HAL_PIXEL_FORMAT_YV12 || handle->format== HAL_PIXEL_FORMAT_YCrCb_420_SP)
    {
       width = ALIGN(handle->width, YV12_ALIGN);
    }else{
       width = ALIGN(handle->width, HW_ALIGN);
    }

    return width * handle->height;
...
static AssignDUETO_T HwcisValidLayer(hwc_layer_1_t *layer)
{
    ...
    if (!HwcValidFormat(handle->format))
    {
        return D_NO_FORMAT;
    }
...
unsigned int setGbufferBlank(unsigned int format, private_handle_t *handle)
{
    ATRACE_CALL();
    int ret = -1;
    ...
    case HAL_PIXEL_FORMAT_AW_YVU_PLANNER420:
    case HAL_PIXEL_FORMAT_AW_YUV_PLANNER420:
        buffer_size[0] = ALIGN(handle->width, YV12_ALIGN) * handle->height; // Y
        buffer_size[1] = ALIGN(handle->width >> 1, YV12_ALIGN) * handle->height; //V&U
        break;
    case HAL_PIXEL_FORMAT_AW_NV21:
    case HAL_PIXEL_FORMAT_AW_NV12:
        buffer_size[0] = ALIGN(handle->width, YV12_ALIGN) * handle->height; // Y
        buffer_size[1] = ALIGN(handle->width, YV12_ALIGN) * handle->height / 2; // UV combine
        break;
    default:
...
bool match_format(const DisplayInfo *psDisplayInfo, layer_info_t *psHwlayer_info, disp_layer_info *layer_info)
{
    ...
    case HAL_PIXEL_FORMAT_AW_YUV_PLANNER420:		// YU12
        ...
        layer_info->fb.size[0].width = ALIGN(handle->width, VE_YV12_ALIGN);
        ...
        layer_info->fb.align[0] = VE_YV12_ALIGN;
        layer_info->fb.align[1] = VE_YV12_ALIGN >> 1;
        layer_info->fb.align[2] = VE_YV12_ALIGN >> 1;
        break;
...
hwc_try_assign_layer(HwcDisContext_t *Localctx, size_t  singcout,int zOrder)
{
    ...
    if((1 == Localctx->psDisplayInfo->VirtualToHWDisplay) && !(CheckVideoFormat(handle->format)))
    {
        Localctx->UsedFB = ASSIGN_GPU;
        dueto = D_SKIP_LAYER;
        goto assign_gpu;
    }
    if((handle->flags& private_handle_t::PRIV_FLAGS_USES_CONFIG) && (!isValidVideoFormat(psLayer->videoFormat)))
    {
    ...
    if(HwcUsageSW(handle) && !CheckVideoFormat(handle->format))
    {
    ...
    if(HwcisBlended(psLayer))
    {
	    ...
	    {
            if(CheckScaleFormat(handle->format))
            {
...
needchannel:
    ...
    tmpCH = Match_nofull_channel(Localctx, -1, isvideo, addLayerCnt, handle->format, WscalFac, HscaleFac, planealpha);
    ...
    {// can assigned to the lowest Channel?
        ...
        if((CH == tmpCH) || (Match_nofull_channel(Localctx, CH, isvideo, addLayerCnt, handle->format, WscalFac, HscaleFac, planealpha) != -1) )
        {
...
assign_overlay:
    ...
    psCH[Localctx->HwCHUsedCnt - CHdiff].iCHFormat = handle->format;
