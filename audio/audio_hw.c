...
#define OUTPUT (2048) // must be 16x, 640x for webrtc
...
struct sunxi_audio_device {
    ...
    struct pcm *out_hdmi_pcm;
    struct pcm *out_codec_pcm;
    struct pcm *out_bt_pcm;
    struct pcm *in_codec_pcm;
    struct pcm *in_bt_pcm;
    //card
    int cardCODEC;
    int cardHDMI;
    int cardSPDIF;
...
void enble_hub_codec(struct mixer *mixer);
void enble_hub_hdmi(struct mixer *mixer);
void enble_hub_bt(struct mixer *mixer);
void enble_hub_codec_in(struct mixer *mixer);
void enble_hub_bt_in(struct mixer *mixer);
void close_hub_codec(struct mixer *mixer);
void close_hub_hdmi(struct mixer *mixer);
void close_hub_bt(struct mixer *mixer);
void close_hub_codec_in(struct mixer *mixer);
void close_hub_bt_in(struct mixer *mixer);
...
static int start_output_stream(struct sunxi_stream_out *out)
{
    ...
    if(adev->out_device & AUDIO_DEVICE_OUT_ALL_SCO)
	{
        ...
        adev->dev_manager[adev->cardBT].flag_exist = 1;
        BTflag = 1;

        out_pcm = pcm_open(adev->cardBT, 0, PCM_OUT, &out->multi_config[card]);
        if (!pcm_is_ready(out_pcm)) {
            ALOGE("cannot open pcm driver: %s", pcm_get_error(out_pcm));
            pcm_close(out_pcm);
            out_pcm = NULL;
        ...
        out->multi_pcm[adev->cardBT] = pcm_open(adev->cardMIX, PORT_BT, PCM_OUT, &out->multi_config[adev->cardBT]);
        ...
        pcm_prepare(out_pcm);
        pcm_prepare(out->multi_pcm[adev->cardBT]);
        ...
        adev->out_bt_pcm = out_pcm;
    }else{
#ifndef KARAOK_AUDIO_DEVICE
    adev->dev_manager[adev->cardBT].flag_out_active = 0;
    for (index = 0; index < MAX_AUDIO_DEVICES; index++)
    {
            ...
            && adev->dev_manager[index].flag_out_active)
        {
            ...
            HDMIflag = 0;
            out_pcm = NULL;
            if(index == adev->cardCODEC){
                CODECflag = 1;
                port = PORT_CODEC;
            }
            if(index == adev->cardHDMI){
                HDMIflag = 1;
                port = PORT_HDMI;
            }
#else
            ...
            set_raw_flag(adev, adev->cardMIX, SND_AUDIO_RAW_DATA_PCM);
            if(CODECflag || HDMIflag ||BTflag)
            {
                if(adev->raw_flag || (out->flags & AUDIO_OUTPUT_FLAG_DIRECT))
                {
                    property_get(PROP_RAWDATA_KEY, prop_value_card, PROP_RAWDATA_DEFAULT_VALUE);
                    if(card == adev->cardHDMI && !strcmp(prop_value_card, PROP_RAWDATA_MODE_HDMI_RAW))
                    {
                        ...
                        out->multi_config[card] = out->config;
                        set_raw_flag(adev, adev->cardMIX, config->raw_flag);
                    }
                }
                out_pcm = pcm_open(card, 0, PCM_OUT, &out->multi_config[card]);
                if (!pcm_is_ready(out_pcm)) {
                    ALOGE("cannot open pcm driver: %s", pcm_get_error(out_pcm));
                    pcm_close(out_pcm);
                    out_pcm = NULL;
                ...
                out->multi_pcm[card] = pcm_open(adev->cardMIX, port, PCM_OUT, &out->multi_config[card]);
                ...
                pcm_prepare(out_pcm);
                pcm_prepare(out->multi_pcm[card]);
                if(HDMIflag)
                    adev->out_hdmi_pcm = out_pcm;
                if(CODECflag)
                    adev->out_codec_pcm = out_pcm;
                if(BTflag)
                    adev->out_bt_pcm = out_pcm;
            }else{
    ...
    enble_hub_codec(adev->audio_hub_mixer);
    enble_hub_hdmi(adev->audio_hub_mixer);
    enble_hub_bt(adev->audio_hub_mixer);
    return 0;
}
...
static int do_output_standby(struct sunxi_stream_out *out)
{
    ...
    if (!out->standby) {
        ...
        if(adev->out_hdmi_pcm)
        {
            pcm_close(adev->out_hdmi_pcm);
            adev->out_hdmi_pcm = NULL;
        }
        if(adev->out_codec_pcm)
        {
            pcm_close(adev->out_codec_pcm);
            adev->out_codec_pcm = NULL;
        }
        if(adev->out_bt_pcm)
        {
            pcm_close(adev->out_bt_pcm);
            adev->out_bt_pcm = NULL;
        }
...
static int start_input_stream(struct sunxi_stream_in *in)
{
    ...
    if(card == adev->cardCODEC)
    {
        adev->in_codec_pcm = pcm_open_req(card, 0, PCM_IN, &in->config, in_ajust_rate);
        if (!pcm_is_ready(adev->in_codec_pcm)) {
            ALOGE("cannot open pcm_in driver: %s", pcm_get_error(adev->in_codec_pcm));
            pcm_close(adev->in_codec_pcm);
            adev->active_input = NULL;
            return -ENOMEM;
        }
        in->pcm= pcm_open_req(adev->cardMIX, 0, PCM_IN, &in->config, in_ajust_rate);
        ...
        pcm_prepare(adev->in_codec_pcm);
        pcm_prepare(in->pcm);
        close_hub_bt_in(adev->audio_hub_mixer);
        enble_hub_codec_in(adev->audio_hub_mixer);
    }
    else if(card == adev->cardBT)
    {
        adev->in_bt_pcm = pcm_open_req(card, 0, PCM_IN, &in->config, in_ajust_rate);
        if (!pcm_is_ready(adev->in_bt_pcm)) {
            ALOGE("cannot open pcm_in driver: %s", pcm_get_error(adev->in_bt_pcm));
            pcm_close(adev->in_bt_pcm);
            adev->active_input = NULL;
            return -ENOMEM;
        }
        in->pcm = pcm_open_req(adev->cardMIX, 0, PCM_IN, &in->config, in_ajust_rate);
        ...
        pcm_prepare(adev->in_bt_pcm);
        pcm_prepare(in->pcm);
        close_hub_codec_in(adev->audio_hub_mixer);
        enble_hub_bt_in(adev->audio_hub_mixer);
    }
    else
    {
        close_hub_bt_in(adev->audio_hub_mixer);
        close_hub_codec_in(adev->audio_hub_mixer);
        in->pcm = pcm_open_req(card, port, PCM_IN, &in->config, in_ajust_rate);
        if (!pcm_is_ready(in->pcm)) {
            ALOGE("cannot open pcm_in driver: %s", pcm_get_error(in->pcm));
            pcm_close(in->pcm);
            adev->active_input = NULL;
...
static int do_input_standby(struct sunxi_stream_in *in)
{
    ...
    if (!in->standby) {
        ...
        if(adev->in_codec_pcm)
        {
            ALOGD("close in codec");
            pcm_close(adev->in_codec_pcm);
            adev->in_codec_pcm = NULL;
        }
        if(adev->in_bt_pcm)
        {
            ALOGD("close in bt");
                                   ...
                                   const char *address __unused)
{
    ...
    if (flags & AUDIO_OUTPUT_FLAG_DIRECT) {
        ...
        out->config.raw_flag = (config->format == AUDIO_FORMAT_AC3)?2:
...
void enble_hub_codec_in(struct mixer *mixer)
{
    ALOGD("enble_hub_codec_in");
    struct mixer_ctl *ctl;
    //i2s3->APBIF_TXDIF0
    ctl = mixer_get_ctl_by_name(mixer, "APBIF0 Src Select");
    mixer_ctl_set_value(ctl, 0, 7);
    //enable i2s3
    ctl = mixer_get_ctl_by_name(mixer, "I2S3IN Switch");
    mixer_ctl_set_value(ctl, 0, 1);
}

void close_hub_codec_in(struct mixer *mixer)
{
    ALOGD("close_hub_codec_in");
    struct mixer_ctl *ctl;
    //i2s3->APBIF_TXDIF0
    ctl = mixer_get_ctl_by_name(mixer, "APBIF0 Src Select");
...
extern int pcm_event_thread_start();
extern void pcm_event_thread_stop();
                     ...
                     hw_device_t** device)
{
    ...
    int card = 0;
    adev->usb_karaok_params.output_precard = adev->cardHDMI;
